...
function labelToKind(lbl: string): PiiKind | null {
  ...
  if (["PHONE", "PHONENUMBER", "TEL", "MOBILE", "CONTACT"].includes(up)) return "PHONE";
  if (["CREDIT_CARD", "CREDITCARDNUMBER", "CARD", "CC"].includes(up)) return "CREDIT_CARD";
  if (["NRIC", "SSN", "ID", "FIN"].includes(up)) return "NRIC";
  if (
    [
      "ADDRESS",
      "ADDR",
  ...
  if (["NAME", "PERSON", "PER", "FIRSTNAME", "LASTNAME", "MIDDLENAME"].includes(up)) return "NAME";
  if (["DOB", "DATE_OF_BIRTH", "BIRTHDATE", "DATE"].includes(up)) return "DOB";
  return null;
}
...
export class DOMWatchers {
  ...
  async init() {
    ...
    this.mo = new MutationObserver((muts) => {
      DEBUG() &&
        log("MutationObserver tick", {added: muts.reduce((n, m) => n + m.addedNodes.length, 0)});
      this.scanEditors();
    });
  ...
  `;
    ...
    `;
      ...
      for (const row of ents) {
        ...
        <label class="pii-item-row">
          ...
          <span class="pii-item-text">${words[row.ent.index]}</span>
        </label>
      `;
        ul.appendChild(li);
      }
      ...
      groupToggle.addEventListener("change", () => {
        section.querySelectorAll<HTMLInputElement>(".pii-item-ck").forEach((ck) => {
          ck.checked = groupToggle.checked;
        });
      });
    }
    ...
    (list.querySelector(".pii-sel-all") as HTMLButtonElement).addEventListener("click", () => {
      list.querySelectorAll<HTMLInputElement>(".pii-item-ck").forEach((ck) => {
        ck.checked = true;
      });
      list.querySelectorAll<HTMLInputElement>(".pii-group-toggle").forEach((ck) => {
        ck.checked = true;
      });
    });
  ...
  private attachEditor(el: HTMLElement) {
    ...
    el.addEventListener("compositionstart", () => {
      this.composing = true;
      log("IME compositionstart");
    });
    el.addEventListener("compositionend", () => {
      this.composing = false;
      log("IME compositionend");
  ...
  private async detectAndPrompt(el: HTMLElement, ctx: EditorCtx) {
    ...
    log(
      "Mapped entities:",
      resp.entities.map((ent) => ({label: ent.label, kind: labelToKind(ent.label)}))
    );
    log("Policy blocked kinds:", this.policy?.blockKinds);
  ...
  private applyRedactions(
    ...
    for (const {ent, kind} of sorted) {
      ...
      // Get the actual text from the input field using the word index
      let actualText = ent.text; // fallback to API text
      if (ent.index >= 0 && ent.index < wordIndices.length) {
        actualText = wordIndices[ent.index]; // use actual text from input
